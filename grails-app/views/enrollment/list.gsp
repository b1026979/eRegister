			...
			<table>
				...
				<g:each in="${enrollmentInstanceList}" status="i" var="enrollmentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${enrollmentInstance.id}">${enrollmentInstance?.regClass?.name.encodeAsHTML()}</g:link></td>
					
						<td>${enrollmentInstance.student.fullStudentName}</td>
