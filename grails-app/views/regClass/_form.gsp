...
<div class="fieldcontain ${hasErrors(bean: regClassInstance, field: 'classInstructor', 'error')} required">
	...
	<g:select id="classInstructor" name="classInstructor.id" from="${uk.ac.shu.webarch.eregister.Instructor.list()}" optionValue="name" required="" value="${regClassInstance?.classInstructor?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: regClassInstance, field: 'course', 'error')} required">
	...
	<g:select id="course" name="course.id" from="${uk.ac.shu.webarch.eregister.Course.list()}" optionValue="courseName" required="" value="${regClassInstance?.course?.id}" class="many-to-one"/>
</div>
...
<g:each in="${regClassInstance?.enrolledStudents?}" var="e">
    <li><g:link controller="enrollment" action="show" id="${e.id}">${e?.student?.fullStudentName.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
